...
#       sh ./run_gromacs_docker_webpage_tests.sh
#
...
echo "======== Section: Showing which GPU running script on =============="
rocminfo | grep gfx
...
echo "======== Section: Rum Gromacs docker adh_dodec benchmark script using MPI in interactive mode =========="
...
echo "========== Section: Run Gromacs docker adh_dodec benchmark script using threaded MPI in interactive mode ============="
docker run --rm -it --device=/dev/kfd --device=/dev/dri --security-opt seccomp=unconfined  amdih/gromacs:2020.3 /bin/bash -c "cd /benchmarks/data/tmpi/adh_dodec; gmx_mpi grompp -f pme_verlet.mdp -c conf.gro -p topol.top -maxwarn 20; gmx mdrun -pin on -nsteps 100000 -resetstep 90000 -ntmpi 2 -ntomp 28 -noconfout -nb gpu -bonded cpu -pme gpu -npme 1 -v -gpu_id 0 -s topol.tpr; gmx_mpi grompp -f pme_verlet.mdp -c conf.gro -p topol.top -maxwarn 20; gmx mdrun -pin on -nsteps 100000 -resetstep 90000 -ntmpi 4 -ntomp 18 -noconfout -nb gpu -bonded gpu -pme gpu -npme 1 -v -gpu_id 01 -s topol.tpr; gmx_mpi grompp -f pme_verlet.mdp -c conf.gro -p topol.top -maxwarn 20; gmx mdrun -pin on -nsteps 100000 -resetstep 90000 -ntmpi 4 -ntomp 18 -noconfout -nb gpu -bonded gpu -pme gpu -npme 1 -v -gpu_id 0123 -s topol.tpr"
echo "========= Section: adh_dodec using threaded MPI Script complete ============"

echo " =========== Section: Run Gromacs STMV benchmark script using threaded MPI in interactive mode ==========="
...
echo "========= Section: STMV Script using threaded MPI complete ==========="

echo "========= Section: Run ADH_DODEC benchmark script using threaded MPI non-interactive mode ==========="
mkdir -p out
...
echo " ======== Section: Run ADH_DODEC becnhmark script in non-interactive mode ======== "
mkdir -p out
docker run --device=/dev/dri --device=/dev/kfd --security-opt seccomp=unconfined --rm -v $(pwd)/out:/out -w /out amdih/gromacs:2020.3 gmx grompp -f /benchmarks/data/tmpi/adh_dodec/pme_verlet.mdp -c /benchmarks/data/tmpi/adh_dodec/conf.gro -p /benchmarks/data/tmpi/adh_dodec/topol.top -maxwarn 20

docker run --device=/dev/dri --device=/dev/kfd --security-opt seccomp=unconfined --rm  -v $(pwd)/out:/out -w /out amdih/gromacs:2020.3 mpirun --allow-run-as-root -np 2 gmx_mpi mdrun -nsteps 100000 -resetstep 90000 -ntomp 24 -noconfout -nb gpu -bonded cpu -pme gpu -npme 1 -v -nstlist 400 -gpu_id 0 -s topol.tpr
