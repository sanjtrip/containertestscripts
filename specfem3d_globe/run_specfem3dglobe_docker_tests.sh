...
docker pull amdih/specfem3d_globe:1ee10977
echo "Pull complete"
...
echo "========== Section: Run Globe convenience benchmark in interactive mode with 6 GPUs ======== "
docker run --rm -it --device=/dev/dri --device=/dev/kfd --security-opt seccomp=unconfined amdih/specfem3d_globe:1ee10977 /bin/bash -c "export OMPI_ALLOW_RUN_AS_ROOT=1; export OMPI_ALLOW_RUN_AS_ROOT_CONFIRM=1; benchmark global_s362ani_shakemovie -o /tmp/out"
echo "============== Section: Convenience script complete ============= "

echo "========== Section: Run Globe example benchmark in interactive mode with 6 GPUs ======== "
docker run --rm -it --device=/dev/dri --device=/dev/kfd --security-opt seccomp=unconfined amdih/specfem3d_globe:1ee10977 /bin/bash -c " cd /opt/specfem3d_globe/EXAMPLES/regional_Greece_small; export OMPI_ALLOW_RUN_AS_ROOT=1; export OMPI_ALLOW_RUN_AS_ROOT_CONFIRM=1; ./run_this_example.sh"
